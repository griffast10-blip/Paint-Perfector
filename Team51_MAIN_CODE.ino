...
int getCurrScreen(int prevScreen)
{
  ...
  // returns the appropriate screen number to currScreen
    ...
    {
        button0 = digitalRead(button0Pin);
      	if (!button0) { while(!digitalRead(button0Pin)); colorSensor(); rgbToHEX(); return 1; }
      	button1 = digitalRead(button1Pin);
...
void rgbTo255(uint16_t R, uint16_t G, uint16_t B, uint16_t C, uint8_t &R8, uint8_t &G8, uint8_t &B8) {
  ...
  float Rn = (float)R / ((float)C*2);
  float Gn = (float)G / ((float)C*2);
  float Bn = (float)B / ((float)C*2);
  ...
  // Clamp to valid range and adjusts blueness
R8 = (uint8_t)constrain((int)(R8 * 1.1), 0, 255);
G8 = (uint8_t)constrain((int)(G8 * 1.0), 0, 255);
B8 = (uint8_t)constrain((int)(B8 * 0.5), 0, 255);
...
void rgbToHEX() {
  uint8_t R8, G8, B8;
  rgbTo255(red, green, blue, clear, R8, G8, B8);
  byte R, G, B;
  ...
  long RGB = ((long)R << 16L | (long)G << 8 | (long)B); // What does this do?
  sprintf(currHex, "#%02X%02X%02X", R, G, B);
